...
*/
GreedyRandomAlg::GreedyRandomAlg(double capacity, double budget, std::vector<std::pair<double, double>> verticesCoordinations, std::vector<double> verticesPrizes, std::vector<double> verticesSizes){
...
////     std::cout<<"\nList of vertices: ";
////     for(int v : G_Vertices) std::cout<< v <<" ";
...
//        weightOfTree = primeMST(S, edgesWeights);
        if (bestPrize < prizeOfTree) {
           ...
           if (sizeOfTree > capacity) 
                dpForSize(S, capacity, verticesCoordinations, verticesPrizes, verticesSizes, sizeOfTree, prizeOfTree, weightOfTree, verticesRespectingCapacity);
           else {
                weightOfTree = findTSPApprox(S, verticesCoordinations);
                verticesRespectingCapacity = S;
           }
           if (sizeOfTree < capacity && weightOfTree < budget) 
                addMoreVertices(vecRatioPrizeCost, verticesRespectingCapacity, sizeOfTree, prizeOfTree, weightOfTree, budget, capacity, S, verticesCoordinations, verticesPrizes, verticesSizes, "GreedyRandom");
