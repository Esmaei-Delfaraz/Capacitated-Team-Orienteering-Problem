...
OutputResults::OutputResults(std::string graphT, int numVertices, int valNumVehicles, std::vector<std::pair<double, double>> valVerticesCoordinations, std::vector<double> valVerticesPrizes, std::vector<double> valVerticesSizes, double valBudget, double valCapacity, int valNumThresholds, int valNumThreads, int solve2Approx, double valSubsetSizeFactor, double factorViolation, std::string outputDirectory){
    ...
    if (solve2Approx == 1)
      for (int k = 0; k < numVehicles; k++) {
          if (verticesCoordinationsOf2Approx.size() > 1) {
             ...
             removeCoveredVertices(verticesOf2Approx, verticesCoordinationsOf2Approx, verticesPrizesOf2Approx, verticesSizesOf2Approx);
             verticesOf2Approx.clear();
             binarySearch(verticesCoordinationsOf2Approx, verticesPrizesOf2Approx, verticesSizesOf2Approx, "2Approx", verticesOf2Approx, costOf2Approx[k], prizeOf2Approx[k], sizeOf2Approx[k]);
   ...
   for (int k = 0; k < numVehicles; k++) {
      if (verticesCoordinationsOfSqrBApprox.size() > 1) {
           ...
           //First remove the covered vertices by previuse vehicle
           removeCoveredVertices(verticesOfSqrBApprox, verticesCoordinationsOfSqrBApprox, verticesPrizesOfSqrBApprox, verticesSizesOfSqrBApprox);
           verticesOfSqrBApprox.clear();
           for (int v : verticesOfSqrBApprox) { 
               verticesPrizesOfSqrBApprox[v] = 0;
               verticesSizesOfSqrBApprox[v] = capacity + 1;
           }
           binarySearch(verticesCoordinationsOfSqrBApprox, verticesPrizesOfSqrBApprox, verticesSizesOfSqrBApprox, "SqrBApprox", verticesOfSqrBApprox, costOfSqrBApprox[k], prizeOfSqrBApprox[k], sizeOfSqrBApprox[k]);
//           std::cout<<"Size of vectors " <<verticesCoordinationsOfSqrBApprox.size() <<" "<< verticesPrizesOfSqrBApprox.size()<<" "<< verticesSizesOfSqrBApprox.size()<<"\n";
   ...
   for (int k = 0; k < numVehicles; k++) {
         if (verticesCoordinationsOfGreedy.size() > 1) {
           ...
           //First remove the covered vertices by previuse vehicle
           removeCoveredVertices(verticesOfGreedy, verticesCoordinationsOfGreedy, verticesPrizesOfGreedy, verticesSizesOfGreedy);
           verticesOfGreedy.clear();
//           for (int v : verticesOfGreedy) { 
//             verticesPrizesOfGreedy[v] = 0;
//             verticesSizesOfGreedy[v] = capacity + 1;
//           }
           binarySearch(verticesCoordinationsOfGreedy, verticesPrizesOfGreedy, verticesSizesOfGreedy, "Greedy", verticesOfGreedy, costOfGreedy[k], prizeOfGreedy[k], sizeOfGreedy[k]);
...
void OutputResults::removeCoveredVertices(std::vector<int> coveredVertices, std::vector<std::pair<double, double>>& verticesCoordinationOfAlg, std::vector<double>& verticesPrizesOfAlg, std::vector<double>& verticesSizeOfAlg){
...
void OutputResults::binarySearch(std::vector<std::pair<double, double>> verticesCoordinationsOfAlg, std::vector<double> verticesPrizesOfAlg, std::vector<double> verticesSizesOfAlg, std::string nameOfAlg, std::vector<int>& verticesOfAlg, double& costOfAlg, double& prizeOfAlg, double& sizeOfAlg) {
...
//           double mst_w = findTSP2Approx(verticesOfG, edgesWeightsOfAlg, verticesCoordinationsOfAlg, verticesServicesTimeOfAlg);
           double tsp_w = findTSPApprox(verticesOfG, verticesCoordinationsOfAlg);
           if (tsp_w <= budget) { 
                  std::cout << "\n-MST weight is at most the budget and is equal to: " << tsp_w <<"\n";
                  
                  //Save the vertices of MST
                  double costOfMST = 0;
                  double prizeOfMST = 0;
                  double sizeOfMST = 0;
//                  double sizeOfMST = sizeTree(mst, verticesSizesOfAlg, tempVerticesOfMST);
                  ...
                  if(sizeOfG <= capacity) {
                       ...
                       dpForSize(verticesOfG, capacity, verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg, sizeOfMST, prizeOfMST, costOfMST, verticesOfMSTRespectingCapacity);
                  }
           ...
           else{//This is the case where there is no tour including all vertices and cover all the vertices
                 if(nameOfAlg == "2Approx"){
                    ...
                    compute2Approx(G, verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg, verticesOf2ApproxTemp, costOf2ApproxTemp, prizeOf2ApproxTemp, sizeOf2ApproxTemp);
                    ...
                    computeSqrBApprox(verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg, verticesOfSqrBApproxTemp, costOfSqrBApproxTemp, prizeOfSqrBApproxTemp, sizeOfSqrBApproxTemp);
                    ...
                    computeGreedy(verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg, verticesOfGreedyTemp, costOfGreedyTemp, prizeOfGreedyTemp, sizeOfGreedyTemp);
...
void OutputResults::compute2Approx(Graph G, std::vector<std::pair<double, double>> verticesCoordinationsOfAlg, std::vector<double> verticesPrizesOfAlg, std::vector<double> verticesSizesOfAlg, std::vector<int>& verticesOf2ApproxTemp, double& costOf2ApproxTemp, double& prizeOf2ApproxTemp, double& sizeOf2ApproxTemp){
     //set the parameters
     double l = 0.0020055;
     double r = 0.0020057;
...
//     costOf2ApproxTemp = approxPrizeCost.second;
     ...
     dpForSize(selectedVerticesOf2Approx, capacity, verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg, sizeOf2ApproxTemp, prizeOf2ApproxTemp, costOf2ApproxTemp, verticesOf2ApproxTemp);
}


void OutputResults::computeSqrBApprox(std::vector<std::pair<double, double>> verticesCoordinationsOfAlg, std::vector<double> verticesPrizesOfAlg, std::vector<double> verticesSizesOfAlg, std::vector<int>& verticesOfSqrBApproxTemp, double& costOfSqrBApproxTemp, double& prizeOfSqrBApproxTemp, double& sizeOfSqrBApproxTemp){

    SqrBApproxAlg sqrB(capacity, budget, verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg, numThresholds, subsetSizeFactor, numThreads);
    ...
    costOfSqrBApproxTemp = findTSPApprox(verticesOfSqrBApproxTemp, verticesCoordinationsOfAlg);//findTSP2Approx(verticesOfSqrBApproxTemp, edgesWeightsOfAlg, verticesCoordinationsOfAlg);
}


void OutputResults::computeGreedy(std::vector<std::pair<double, double>> verticesCoordinationsOfAlg, std::vector<double> verticesPrizesOfAlg, std::vector<double> verticesSizesOfAlg, std::vector<int>& verticesOfGreedyTemp, double& costOfGreedyTemp, double& prizeOfGreedyTemp, double& sizeOfGreedyTemp) {

    GreedyAlg greedyAlg(capacity, budget, verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg, numThreads);
    ...
    costOfGreedyTemp = findTSPApprox(verticesOfGreedyTemp, verticesCoordinationsOfAlg);//findTSP2Approx(verticesOfGreedyTemp, edgesWeightsOfAlg, verticesCoordinationsOfAlg);

}

void OutputResults::computeGreedyRandom(std::vector<std::pair<double, double>> verticesCoordinationsOfAlg,  std::vector<double> verticesPrizesOfAlg, std::vector<double> verticesSizesOfAlg, std::vector<int>& verticesOfGreedyRandomTemp, double& costOfGreedyRandomTemp, double& prizeOfGreedyRandomTemp, double& sizeOfGreedyRandomTemp){
//    std::cout<<"\nBefore GreedyRandom\n";
    GreedyRandomAlg greedyRandomAlg(capacity, budget, verticesCoordinationsOfAlg, verticesPrizesOfAlg, verticesSizesOfAlg);
//    std::cout<<"\nAfter GreedyRandom\n";
    //Get the selected vertices in SqrBApproxAlg before appplying the capacity constraint
    ...
    costOfGreedyRandomTemp = findTSPApprox(verticesOfGreedyRandomTemp, verticesCoordinationsOfAlg);//findTSP2Approx(verticesOfGreedyRandomTemp, edgesWeightsOfAlg, verticesCoordinationsOfAlg);
