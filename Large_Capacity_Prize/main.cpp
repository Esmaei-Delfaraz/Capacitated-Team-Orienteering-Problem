...
void Read_OutPut(std::string inputFileName, std::string graphT, int numVehicles, double budget, double capacity, int numThresholds, int numThreads, int solve2Approx, double subsetSizeFactor, double factorViolation, std::string outputDirectory){
         ...
         std::vector<double> verticesServicesTime;
         std::vector<std::pair<double, double>> verticesCoordinations;
         int numVertices = graphFromFile(inputFileName, verticesCoordinations, verticesSizes, verticesPrizes, verticesServicesTime, numVehicles, budget, capacity);
         std::cout<<"\nBudget is : "<< budget;
         std::cout<<"\nCapacity is : "<< capacity;
         std::cout<<"\nnumber of vehicles is : "<< numVehicles;
         std::cout<<"\n";
         /* 
...
//         std::cout<<"\nTotal prize is : "<<totalPrize <<"\n";
         for(int i = verticesPrizes.size() - 1; i >= 1; i--) {
                ...
                if(2*dist + verticesServicesTime[i] > budget){
                   ...
                   verticesServicesTime.erase(verticesServicesTime.begin() + i);
                }
         }    
         ...
         if(!verticesSizes.empty() && !verticesPrizes.empty() && !verticesCoordinations.empty()){
               OutputResults ORClass(graphT, numVertices, numVehicles, verticesCoordinations, verticesPrizes, verticesSizes, verticesServicesTime, budget, capacity, numThresholds, numThreads, solve2Approx, subsetSizeFactor, factorViolation, outputDirectory);
         }   
         else{
...
int main(int argn, char *argv[]){
  ...
  int n, k, solve2Approx = 0, numVehicles = 0, numThreads, numThresholds, minPrize, maxPrize;
  double p = 1, a, b, minWeight, maxWeight, budget = 0, capacity = 0, subsetSizeFactor = 1, factorViolation = 0;
  int c=1;
